...
GatewayRequest::make_request(const string &url, string &type)
{
    ...
    if (type.empty() || type == "gateway") {
        ...
        if (!disp.empty()) {
            ...
            BESDEBUG("gateway", "Looked at disposition " << disp << " for type, returned \"" << type << "\"" << endl);
        }
        ...
        if (type.empty() && !ctype.empty()) {
            ...
            BESDEBUG("gateway", "Looked at content type " << ctype << " for type, returned \"" << type << "\"" << endl);
        }
        ...
        if (type.empty()) {
            ...
            BESDEBUG("gateway", "Looked at url " << url << " for type, returned \"" << type << "\"" << endl);
        }
